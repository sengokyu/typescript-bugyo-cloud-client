...
const formatTag =
  (strings: TemplateStringsArray, ...keys: Array<number>): formatter =>
  (...values) =>
    strings[0] + keys.map((key, i) => values[key] + strings[i + 1]).join("");

export const USER_AGENT = "Mozilla 5.0 ()";
export const URL_TEMPLATE: { [k in string]: formatter } = {
  ...
  WorkflowOvertimePage: formatTag`https://hromssp.obc.jp/${0}/${1}/Workflow/Overtime/`,
  WorkflowApply: formatTag`https://hromssp.obc.jp/${0}/${1}/Workflow/Apply/`,
};

export type EndpointName = keyof typeof URL_TEMPLATE;
